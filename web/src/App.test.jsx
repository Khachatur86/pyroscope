...
describe("Capture compare", () => {
  ...
  it("keeps a history of recent comparisons", async () => {
    const responses = [
      {
        baseline: { session_name: "fixture-a" },
        candidate: { session_name: "fixture-b" },
    ...
    const firstBaseline = new File(["{}"], "baseline-a.json", { type: "application/json" });
    const firstCandidate = new File(["{}"], "candidate-b.json", { type: "application/json" });
    const secondBaseline = new File(["{}"], "baseline-c.json", { type: "application/json" });
    const secondCandidate = new File(["{}"], "candidate-d.json", { type: "application/json" });

    fireEvent.change(baselineInput, { target: { files: [firstBaseline] } });
    fireEvent.change(candidateInput, { target: { files: [firstCandidate] } });
    fireEvent.click(compareButton);
    await screen.findByText("fixture-a");

    fireEvent.change(baselineInput, { target: { files: [secondBaseline] } });
    fireEvent.change(candidateInput, { target: { files: [secondCandidate] } });
    fireEvent.click(compareButton);
  ...
  it("clears compare history and persisted state", async () => {
    const compareResponse = {
      baseline: { session_name: "fixture-a" },
      candidate: { session_name: "fixture-b" },
      counts: {
    ...
    fireEvent.click(within(comparePanel).getByRole("button", { name: /clear history/i }));

    expect(within(comparePanel).queryByText("Recent comparisons")).not.toBeInTheDocument();
  ...
  it("reuses a history entry as candidate for a new comparison", async () => {
    const responses = [
      {
        baseline: { session_name: "fixture-a" },
        candidate: { session_name: "fixture-b" },
      ...
      {
        baseline: { session_name: "fixture-z" },
        candidate: { session_name: "fixture-b" },
        counts: {
          baseline_tasks: 6,
          candidate_tasks: 3,
